...
/** Downloads the whole library as `arkivel-library.json`. */
export async function GET() {
  return mediaRoute(async () =>
    NextResponse.json(await readLibrary(), { headers: { "Content-Disposition": 'attachment; filename="arkivel-library.json"' } }),
  );
}
