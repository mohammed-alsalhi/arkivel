...
export default function LibraryBackup({ canEdit, onPendingChange }: { canEdit: boolean; onPendingChange: (pending: boolean) => void }) {
  ...
  async function review(file: File) {
    ...
    try {
      if (!file.name.toLowerCase().endsWith('.json') || file.size > 10_000_000) throw new Error('Choose a library JSON backup under 10 MB.')
      let source: unknown
      try { source = JSON.parse(await file.text()) } catch { throw new Error('This file is not valid JSON. Choose a downloaded library backup.') }
    ...
    } catch (cause) { setError(cause instanceof TypeError ? 'Could not connect to your library. Choose the backup again to retry.' : cause instanceof Error ? cause.message : 'Could not read this backup.') }
  ...
  async function restore() {
    ...
    } catch (cause) { setError(cause instanceof TypeError ? 'Could not connect to your library. Your backup is still ready; try again.' : cause instanceof Error ? cause.message : 'Could not restore this backup. You can try again.') }
  ...
  return <section aria-label="Move your library" className="space-y-3">
    ...
    {canEdit && !counts && <Button variant="outline" className="w-full" disabled={pending} onClick={() => input.current?.click()}><Upload strokeWidth={1.5} />{pending ? 'Checking backup…' : 'Restore a library backup'}</Button>}
    <input ref={input} type="file" accept=".json,application/json" aria-label="Library backup" className="hidden" disabled={pending || !canEdit} onChange={(event) => { const file = event.target.files?.[0]; if (file) void review(file) }} />
    {canEdit && !counts && <p className="text-xs leading-5 text-muted-foreground">Move titles, tags and episode progress between computers. library JSON · under 10 MB.</p>}
