  ...
  return (
    <>
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-56 flex-col border-r border-border bg-[#151517] px-4 py-8 md:flex">
        <Link href="/" aria-label={`${config.name} home`} className="flex items-center gap-3 px-2">
          <Asterisk size={38} strokeWidth={1.5} className="shrink-0 text-primary" aria-hidden="true" />
          <span><span className="text-[17px] font-medium tracking-[0.28em]">{config.name}</span><span className="mt-1 block text-[10px] tracking-normal text-muted-foreground">A little less scrolling.</span></span>
        </Link>
      ...
      <header className="absolute top-0 right-0 left-0 z-30 flex h-[76px] items-center justify-between gap-3 px-5 md:left-56 md:px-8 lg:px-10">
        <Link href="/" className="flex items-center gap-2 text-sm font-medium tracking-[0.15em] md:hidden"><Asterisk className="text-primary" aria-hidden="true" />{config.name}</Link>
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setInfoOpen(true)} aria-label="Library settings" className="hidden size-10 items-center justify-center rounded-full border border-border text-sm md:flex">{config.name.charAt(0)}</button>
