...
Choose any skin independently: `folio`, `wiki`, `editorial`, or `compact` with `NEXT_PUBLIC_ARKIVEL_SKIN`. Settings and the command palette switch the reader preference; light and dark themes remain separate. Editorial fills the viewport with warm paper tones and serif article text; compact uses tighter spacing, a narrower sidebar, and denser tables. Touch controls retain their minimum size.
